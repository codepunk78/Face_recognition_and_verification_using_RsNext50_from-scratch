...
import pathlib
import os
from p1 import IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS,BATCH_SIZE, train_tfrecord, valid_tfrecord, test_tfrecord
from p4 import get_parsed_dataset
...
def get_images_and_labels(data_root_dir):
    ...
    for file in os.listdir(data_root):
        file_path = pathlib.Path(os.path.join(data_dir, file))
        img_str=[str(path) for path in list(file_path.glob('*/*'))]
        for x in img_str:
            all_image_path.append(x)
    # get labels' names
    label_names = sorted(item.name for item in data_root.glob('*/'))
    # dict: {label : index}
    label_to_index = dict((label, index) for index, label in enumerate(label_names))
    # get all images' labels
    all_image_label = [label_to_index[pathlib.Path(single_image_path).parent.parent.name] for single_image_path in all_image_path]
